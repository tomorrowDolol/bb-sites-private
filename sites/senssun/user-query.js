/* @meta
{
  "name": "senssun/user-query",
  "description": "查询香山大数据平台用户信息与绑定设备",
  "domain": "inside.senssun.com",
  "args": {
    "loginName": { "required": true, "description": "登录账号，通常是手机号" },
    "appId": { "required": false, "description": "指定应用 ID；默认取当前页面选择的 appId" }
  },
  "capabilities": ["query", "account", "device"],
  "readOnly": true,
  "example": "bb-browser site senssun/user-query --loginName 13180121679 --appId APP-ZL-202008151020"
}
*/
async function (args) {
  ...
  function normalizeDevice(device) {
    return {
      ...
      unit: device?.unit == null ? null : toNumber(device.unit),
      keyWords: device?.keyWords || null,
  ...
  try {
    ...
    if (!loginName) {
      throw makeError("缺少必填参数 loginName", {
        status: 400,
        code: "missing_login_name",
        hint: "示例：bb-browser site senssun/user-query --loginName 13180121679"
      });
    }
    ...
    if (!selection.appId) {
      throw makeError("当前账号没有可用应用，无法执行用户查询", {
        status: 404,
        code: "missing_app",
        data: {
          availableApps: apps,
