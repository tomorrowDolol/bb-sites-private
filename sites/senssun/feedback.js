/* @meta
{
  "name": "senssun/feedback",
  "description": "查询香山大数据平台用户反馈列表",
  ...
  "capabilities": ["query", "feedback"],
  "readOnly": true,
  "example": "bb-browser site senssun/feedback 0 10 --appId APP-ZL-202008151020"
}
*/
async function (args) {
  const loginHint = "请先在当前浏览器登录香山大数据平台，再重试。目标页面通常是 http://inside.senssun.com/bigdata/index.html#/data-cloud/feedback 。";
  ...
  function normalizeFeedback(item) {
    const images = parseJson(item?.imageList, []);
    const imageList = Array.isArray(images) ? images.filter(Boolean).map(String) : [];
    return {
      uuid: item?.uuid || null,
      ...
      hasErrorLog: Boolean(trimText(item?.errorLogId)),
      imageList,
  ...
  try {
    ...
    if (!selection.appId) {
      throw makeError("当前账号没有可用应用，无法执行反馈查询", {
        status: 404,
        code: "missing_app",
        data: {
          availableApps: apps,
    ...
    const response = await api("/admin/v1/api/user/feedback/pageSearch", {
      query: {
    ...
    const list = Array.isArray(response?.data) ? response.data.map(normalizeFeedback) : [];
    ...
    return {
      ...
      pagination: {
        pageIndex,
        pageSize,
        count: toNumber(response?.count, list.length),
    ...
    return {
      error: error?.message || "查询香山用户反馈失败",
      code: error?.code,
      status: error?.status,
