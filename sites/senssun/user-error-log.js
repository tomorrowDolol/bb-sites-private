/* @meta
{
  "name": "senssun/user-error-log",
  "description": "查询香山大数据平台用户错误日志",
  "domain": "inside.senssun.com",
  "args": {
    "errorLogId": { "required": false, "description": "错误日志 ID" },
  ...
  "capabilities": ["query", "logs", "account"],
  "readOnly": true,
  "example": "bb-browser site senssun/user-error-log --errorLogId 2c0377fd-ce6f-440e-9fcf-5c3ad0580de3 --appId APP-ZL-202008151020"
}
*/
async function (args) {
  const loginHint = "请先在当前浏览器登录香山大数据平台，再重试。目标页面通常是 http://inside.senssun.com/bigdata/index.html#/data-cloud/user-error-log 。";
  ...
  try {
    const errorLogId = trimText(args.errorLogId);
    ...
    if (!selection.appId) {
      throw makeError("当前账号没有可用应用，无法执行用户错误日志查询", {
        status: 404,
        code: "missing_app",
        data: {
          availableApps: apps,
    ...
    const response = await api("/admin/v1/api/user/failed/log/pageSearch", {
      query: {
        errorLogId,
        accountId,
    ...
    return {
      error: error?.message || "查询香山用户错误日志失败",
      code: error?.code,
      status: error?.status,
