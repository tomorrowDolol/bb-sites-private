/* @meta
{
  "name": "tabcode/key-priority",
  "description": "查询或设置 TabCode 扣费优先级",
  "domain": "tabcode.cc",
  "args": {
    "priority": { "required": false, "description": "可选值：plan_first 或 topup_first" }
  },
  "capabilities": ["api-keys", "billing"],
  "readOnly": false,
  "example": "bb-browser site tabcode/key-priority --priority topup_first"
}
*/
async function (args) {
  ...
  try {
    ...
    if (!priority) {
      const current = await api("/v1/me/key-priority");
      return { priority: current?.priority || "plan_first" };
    }

    if (!["plan_first", "topup_first"].includes(priority)) {
      return { error: "priority 只支持 plan_first 或 topup_first" };
    }

    await api("/v1/me/key-priority", {
      method: "PATCH",
      json: { priority }
    });
