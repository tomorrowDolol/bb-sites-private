/* @meta
{
  "name": "tabcode/key-update",
  "description": "更新 TabCode API Key 名称或状态",
  "domain": "tabcode.cc",
  "args": {
    "id": { "required": true, "description": "Key ID" },
    "name": { "required": false, "description": "新的 Key 名称" },
    "status": { "required": false, "description": "新的状态：active 或 disabled" }
  },
  "capabilities": ["api-keys"],
  "readOnly": false,
  "example": "bb-browser site tabcode/key-update <id> --name prod --status active"
}
*/
async function (args) {
  ...
  try {
    ...
    if (args.name != null && String(args.name).trim()) {
      payload.name = String(args.name).trim();
    }

    if (args.status != null && String(args.status).trim()) {
      const status = String(args.status).trim();
      if (!["active", "disabled"].includes(status)) {
        return { error: "status 只支持 active 或 disabled" };
      }
      payload.status = status;
    }

    if (Object.keys(payload).length === 0) {
      return { error: "至少提供一个可更新字段：name 或 status" };
    }
    ...
    return {
      error: error?.message || "更新 API Key 失败",
      code: error?.code,
      status: error?.status,
